...
useEffect(() => {
  document.body.classList.toggle('enable-scrolling', isActive);
  var geSearch = document.getElementById("ge-search-component");  
  geSearch.classList.toggle('active', isActive);
  var geMain = document.getElementById("ge-main");  
  geMain.classList.toggle('hide-element', isActive);
  return () => 
  ...
  <nav id="block-ge-unified-main-menu" className="contextual-region navbar navbar-dark navbar-expand-lg ge-menu-main bg-transparent text-white px-3 px-lg-4" data-block-plugin-id="system_menu_block:main">
    <div className="container-fluid">
      ...
      <ul className="nav d-lg-none ge-nav-icons-mobile">
        ...
        <li className="nav-item icon pl-3 search-icon" onClick={() => setActive(!isActive)}>
          <a href="javascript:void(0);"  className={`${isActive ? "search-open active-nav" : "search-close"} nav-link px-3 text-white enable`} id="ge-search-component-init">
      ...
      {/* mobile Nav */}
      <div className={`panel ${isOpen ? "nav-open active-nav" : "nav-close"}`} >
          ...
          <div className="inner--wrapper" >
            <ul className="list-unstyled nav--list--level1 col-md-6 col-sm-12">
              <li className="nav-item ">
                <Link href="/news" >
                  <a target="_self" onClick={() => setOpen(!isOpen)} className="nav-link px-auto px-lg-3 text-white" title="News"><h3>News
                    </h3></a>
                </Link>
              </li>
              <li className="nav-item ">
                <Link href="/blogs">
                  <a target="_self" onClick={() => setOpen(!isOpen)} className="nav-link px-auto px-lg-3 text-white" title="Blogs"><h3>Blogs</h3></a>
                </Link>
              </li>
              <li className="nav-item ">
                <Link href="/reports">
                  <a target="_self" onClick={() => setOpen(!isOpen)} className="nav-link px-auto px-lg-3 text-white" title="Reports"><h3>Reports</h3></a>
                </Link>
              </li>
              <li className="nav-item ">
                <Link href="/about-us">
                  <a target="_self" onClick={() => setOpen(!isOpen)} className="nav-link px-auto px-lg-3 text-white" title="About us"><h3>About us</h3></a>
                </Link>
              </li>
      ...
      <ul className="nav d-none d-lg-flex ge-nav-icons-desktop">
        ...
        <li className="nav-item icon search-icon" onClick={() => setActive(!isActive)} >
          
					<a id="ge-search-component-init" className={`panel ${isActive ? "search-open active-nav" : "search-close"} nav-link enable`}>
						<i className="ficon-search"><span className="hidden">Search</span></i>
					</a>
